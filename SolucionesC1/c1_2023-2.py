...
def repetir(seccion): #Código hecho en clases en conjunto con los estudiantes
    primer_indice = misterio(seccion, ":", 1)
    segundo_indice = misterio(seccion, ";", 1)

    #Se corregía el error previo con esta simple comprobación
    if len(seccion[primer_indice + 1: segundo_indice]) == 1:
        numero = int(seccion[primer_indice + 1: segundo_indice])
    else:
        numero = int(seccion[primer_indice + 1: segundo_indice - 1])
