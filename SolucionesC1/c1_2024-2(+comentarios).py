...
def comer(plantacion, plan, inicio):
    #Notar que esta función debe hacer varias cosas, además de validaciones. Un paso muy importante es tener claro como queremos implementar la solucion: ¿While? ¿For?, también que sepamos las variables que queremos utilizar.

    #Sabemos que el bucle principal si o si debe ser el del plan de alimentación del picaflor, nuestra función debe terminar a penas terminamos de leer dicho dato -> Lo más comodo  es recorrerlo con un while. Esto porque el while nos va a permitir comparar el caracter actual con el anterior o siguiente, algo que nos ayudará bastante para la implementación.

    #Nos dicen que la función promediar_comidas() puede ser útil entender su funcionamiento, dado que notar que leen el plan de 4 en 4, para cada iteracción tener una instrucción completa.

    #Entonces necesitamos una variable auxiliar f igual que la funcion misterio

    #Necesitamos una variable que nos diga cuantas flores pudo comer el ave.

    #Necesitamos si o si una variable que nos indique la posición actual del ave

    flores_comidas = 0
    i = 0 #Variable para el while
    ...
    while i < len(plan):
        ...
        i += 4 #Aumentamos en 4 para leer una intrucción completa
